...
var orderItemCollection *mongo.Collection = database.OpenCollection(database.Clinet, "orderItem")
...
func ItemsByOrder(id string) (orderItems []primitive.M, err error) {
	...
	matchStage := bson.D{{Key: "$match", Value: bson.D{{"order_id", id}}}}

	lookupStage := bson.D{{Key: "$lookup", Value: bson.D{{Key: "from", Value: "food"}, {"localField", "food_id"}, {Key: "foreignField", Value: "food_id"}, {Key: "as", Value: "food"}}}}

	unwindStage := bson.D{{"$unwind", bson.D{{"path", "$food"}, {"preserveNullAndEmptyArrays", true}}}}

	lookupOrderStage := bson.D{{Key: "$lookup", Value: bson.D{{"from", "order"}, {"localfield", "order_id"}, {"foreignField", "order_id"}, {"as", "order"}}}}
...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured during the finding of the order item "})
			return
		}
...
func CreateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, orderItem := range orderItemPack.Order_items {
			...
			orderItem.Created_at, _ = time.Parse(time.RFC1123, time.Now().Format(time.RFC3339))
			orderItem.Updated_at, _ = time.Parse(time.RFC1123, time.Now().Format(time.RFC3339))
